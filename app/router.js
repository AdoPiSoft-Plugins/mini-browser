...
function setupProxy(url_str) {
  if (!url_str.match(/^http/gi)) url_str = "http://" + url_str;
  var url;
  try {
    url = new URL(url_str)
  } catch (e) {}
  ...
  proxy = httpProxy(url.host, {
    ...
    proxyReqOptDecorator: function(proxyReqOpts, originalReq) {
      ...
      if (originalReq.headers.referer) {
        ...
        ref_url.port = "";
        if (url.port != "80") ref_url.port = url.port;
...
router.use((req, res, next) => {
  try {
    ...
    var is_mini_browser = (req.headers.cookie || "").match(/mini_browser\=true/gi);
    if (!is_mini_browser) return next();
    if (!proxy) {
